...
class CurlInterceptor extends Interceptor {
  ...
  Future<void> onRequest(
    ...
    try {
      ...
      // String curlCommand = cURLRepresentation(options);
      // options.extra['curlCommand'] = curlCommand;
      //
      // DateTime now = DateTime.now();
      // String formattedDate = DateFormat('dd/MM/yyyy HH:mm:ss').format(now);
      // String formattedDateTimeStamp = DateFormat('yyyyMMddHHmmss').format(now);
      // var sentryId = await Sentry.captureMessage(
      //   'CURL_$formattedDate => ${options.uri.toString()}', // message parameter
      //   level: SentryLevel.error,
      //   params: [curlCommand],
      //   hint: Hint(), // hint parameter (optional)
      //   withScope: (scope) {
      //     // Add unique details to the scope to ensure the event is not grouped
      //     scope.setTag('unique-id', formattedDateTimeStamp);
      //     scope.setExtra('timestamp', formattedDateTimeStamp);
      //     scope.setTag('example-tag', 'tag-value');
      //   },
      // );
    } catch (e) {
...
String cURLRepresentation(RequestOptions options, bool notIncludeAuthor) {
