...
class LoginCubit extends Cubit<LoginState> {
  ...
  void showPassword(bool isShow) => safeEmit(LoginState.showPassword(isShow));
  ...
  Future<void> handleLogin(String username, String password) async {
    ...
    response.fold(
      (l) => safeEmit(LoginState.error(l.errorMessage)),
      (r) async {
        if (r.isNullOrEmpty) {
          safeEmit(LoginState.error(VOLocale.current.login_failed));
        } else {
          if (r?.token != null) {
            saveToken(r?.token);
